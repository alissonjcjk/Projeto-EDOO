...
void ClienteControlador::criar(http_request request) {
    request.extract_json().then([this, request](pplx::task<json::value> task) mutable {
        try {
            ...
            Cliente cliente(nome, email, telefone, Servicos{});

            int id = servico.cria(cliente, servicoPreferido);
            if (id <= 0) {
...
void ClienteControlador::buscarPorId(http_request request) {
    ...
    try
    {
        ...
        out[U("servico_preferido")] = json::value::string(utility::conversions::to_string_t(cliente.getServicoPreferidoNome()));
...
void ClienteControlador::findAll(http_request request) {
    ...
    for (auto& p : pessoas) {
        ...
        result[i++] = j;
        j[U("servico_preferido")] = json::value::string(utility::conversions::to_string_t(p.getServicoPreferidoNome()));
        result[i++] = j;
    }
...
void ClienteControlador::deletar(http_request request) {
    auto paths = uri::split_path(uri::decode(request.relative_uri().path()));
    if (paths.size() < 2) {
    ...
    try {
        int resultado = servico.deletar(id);
        if (resultado <= 0) {
            request.reply(status_codes::NotFound, U("Cliente não encontrado ou falha ao deletar."));
            return;
        }

        json::value out = json::value::object();
        out[U("mensagem")] = json::value::string(U("Cliente deletado com sucesso."));
        request.reply(status_codes::OK, out);
    }
    catch (const std::exception& e) {
        request.reply(status_codes::InternalError, utility::conversions::to_string_t(e.what()));
        ...
        request.reply(status_codes::InternalError, U("Erro desconhecido ao deletar cliente."));
    }
}